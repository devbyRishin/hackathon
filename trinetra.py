...
TWILIO_SID = 'ACd4b4091c27041e55942d7b7155590151'
TWILIO_AUTH = '3e3291be88de3f216c8d12367f04497a'
TWILIO_PHONE = '+17173409071'
TARGET_PHONE = '+919234465617'
try:
...
alert_cooldown = 90  
last_alert_time = 0
SKELETON = [(0,1),(0,2),(1,3),(2,4),(5,6),(5,7),(7,9),(6,8),(8,10),(11,12),(5,11),(6,12),(11,13),(13,15),(12,14),(14,16)]
    ...
    while True:
        ...
        if last_results and last_results.keypoints is not None:
            ...
            for i, box in enumerate(last_results.boxes):
                ...
                roi = gray[max(0,y1):min(480,y2), max(0,x1):min(640,x2)]
                ...
                for view in [normal_view, night_vision, thermal_view]:
                    ...
                    if view is thermal_view:
                        cv2.putText(view, f"{sim_temp:.1f}C", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        ...
        res_size = (400, 300)
        combined = np.hstack((cv2.resize(normal_view, res_size), cv2.resize(night_vision, res_size), cv2.resize(thermal_view, res_size)))
        cv2.rectangle(combined, (0, 0), (1200, 40), (20, 20, 20), -1)
        hud_label = f"SAR SCAN | SURVIVORS: {survivor_count} | LOC: 20.9467 N, 72.9520 E"
